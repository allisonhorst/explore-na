...
These are selected examples for Allison's talk 2020-05-19 talk *How to become a missing explorer: including missingness in exploratory data analysis* at the Bren School of Environmental Science and Management (UC Santa Barbara). 
...
Use `gg_miss_fct()` to create a heatmap of % missings by variable and factor level:

```{r gg_miss_fct_meds, exercise = TRUE}
gg_miss_fct(kp_rodents, fct = common_name)
```
Try updating the factor to a different variable, like `age` or `sex`, to see how your heatmap changes!

##### `r fa("fas fa-robot", fill = "orange")` MEDS Talk Example 3: gg_miss_upset()

Use `gg_miss_upset()` to create an UpSet plot of missing intersections. 
