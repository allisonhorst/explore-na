...
Find the percent of values that are `NA` for the entire `kp_rodents` data frame.

```{r pct_miss_1, exercise = TRUE}
pct_miss(kp_rodents)
```
So `r round(pct_miss(kp_rodents), 1)`% of all cells in `kp_rodents` contain `NA`.

##### `r fa("fas fa-keyboard", height = 25, fill = "orange")` Code exercise: pct_miss( )

What percent of values in the `ear_length` column of `kp_rodents` are `NA`?
...
##### `r fa("fas fa-robot", height = 25, fill = "orange")` Example 6.1
...
We'll use `gg_miss_point()` to visualize which non-missing values of one variable have `NA`s for another variable. 
